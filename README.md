...
## 개요
* 사고, 공사, 행사 등과 관련된 긴급 교통정보를 사용자가 입력하고 기존 긴급교통정보 데이터와 병합하여 DB에 저장이 되면 이를 통해 지역 본부별로 다량의 데이터가 확보될 것임. 또한 이후 저장된 데이터는 사용자가 필요한(긴급) 정보를 관리 및 조회할 수 있도록 웹페이지와 DB를 구현하였음.
